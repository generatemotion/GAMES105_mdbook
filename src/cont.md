...
> &#x2705; 一般生成式模型是这样的形式：从一个简单的 PDF，通过 \\(f(z)\\)，映射到 \\(p(x)\\).   
> &#x2705; 关键是 \\(f(z)\\) 要学好。  
...
> &#x2705; VAE：已知一些真实数据采样，用 Encoder 编码到简单分布上的点，再用Decoder 变回原分布上的点。  
> &#x2705; GAN：无 Encoder，增加一个判别器。   
